...
class Solution:
    ...
    #        steps = 0
    #        while dq:
    #            print(f'\tdq, steps = {dq}, {steps}')
    #            size = len(dq)
    #            for _ in range(size):
    #                curr = dq.popleft()
    #                if curr == n - 1:
    #                    return steps
    #                for next_node in self.hsh[curr]:
        ...
        def bfs():
            ...
            while dq:
                ...
                for _ in range(size):
                    ...
                    if curr == n - 1:
                        return distance
                    for next_node in roads[curr]:
                        if next_node not in seen:
                            dq.append(next_node)
                            seen.add(next_node)
                distance += 1
        roads = [[i+1] if i < n-1 else [] for i in range(n)]
        res = list()
        for u, v in queries:
            roads[u] += [v]
            min_distance = bfs()
