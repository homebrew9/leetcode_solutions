# =========================================================================
# Very messy code! Couldn't implement Dijkstra's algorithm correctly.
# It should be used here, but cannot figure out the correct way to do it
# when the graph is bidirectional.
# The last test case throws TLE. The answer (33) is correct, but it is
# way too slow!
# =========================================================================
    ...
    def findTheCity(self, n: int, edges: List[List[int]], distanceThreshold: int) -> int:
        ...
        for i in range(n):
            ...
            while h:
                size = len(h)
                #print('====')
                #print(h, size, visited)
                for _ in range(size):
                    dist_so_far, node = heapq.heappop(h)
                    #print(f'\tdist_so_far, node = {dist_so_far}, {node}')
                    for next_node, dist in hsh[node]:
                        if next_node != i and (dist_so_far + dist, next_node) not in visited \
                        and dist_so_far + dist <= distanceThreshold:
                            heapq.heappush(h, (dist_so_far + dist, next_node))
                            visited.add((dist_so_far + dist, next_node))
                            neighbors[i].add(next_node)
                    #print(f'\t\th, visited = {h}, {visited}')
        #print(neighbors)
        return sorted([(k, len(v)) for k, v in neighbors.items()], key=lambda x: (x[1], -x[0]))[0][0]
