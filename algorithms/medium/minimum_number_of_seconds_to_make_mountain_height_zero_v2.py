...
class Solution:
    def minNumberOfSeconds(self, mountainHeight: int, workerTimes: List[int]) -> int:
        ...
        # A slight variation - double binary search instead of quadratic formula.
        def max_height_destroyed(t, w):
            # For a given time t and workerTime w, find the maximum height x destroyed
            # by the worker. Here, x^2 + x = 2*t/w
            left, right = 0, mountainHeight
            while left <= right:
                mid = (left + right) // 2
                if mid * mid + mid <= 2 * t / w:
                    left = mid + 1
                else:
                    right = mid - 1
            return right
