...
class Solution:
    # This version resulted in TLE as well!!
    def findEvenNumbers(self, digits: List[int]) -> List[int]:
        arr = list()
        for item in list(itertools.permutations(digits, 3)):
            if item[-1] in (0,2,4,6,8) and item[0] != 0:
                #print(f'\t>>> item = {item}')
                arr.append(int(''.join([str(i) for i in item])))
