...
class Solution:
    def putMarbles(self, weights: List[int], k: int) -> int:
        # If k = 1, then put we only have 1 bag, and all marbles in them.
        # So max score = min score and hence diff = 0. If k > 1, then keep in mind
        # that the 1st and last element will be included in both scores (max and min), and
        # they will be canceled out in the diff. So we just have to add the elements at
        # the boundary of each cut to get the score. To determine max or min, we can either
        # use heap or just sort the cuts array. Example: weights = [1,3,5,1,2,4,1,3], k = 3
        # For max score, cut at indices 2 and 5 => [1, 3] [5, 1,  2] [4, 1,  3] => 3+5 + 2+4 = 14
        # For min score, cut at indices 4 and 7 => [1, 3,  5, 1] [2,  4, 1] [3] => 1+2 + 1+3 =  7
        # Diff = 14 - 7 = 7. We do not have to include 1 and 3 since they will get canceled.
        if k == 1:
            return 0
        cuts = list()
        ...
        cuts.sort()
        max_score = sum(cuts[-(k-1):])
        min_score = sum(cuts[:(k-1)])
